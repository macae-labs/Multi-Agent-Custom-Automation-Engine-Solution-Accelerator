...
class FirestoreHealthChecker(BaseProviderHealthChecker):
    ...
    async def check_health(self) -> ProviderHealthMetrics:
        ...
        async def test_firestore_comprehensive():
            ...
            try:
                ...
                if list_result.success:
                    ...
                    if len(cols) > 100:
                        logger.warning(
                            f"Firestore quota: {len(cols)} collections > 100"
                        )
            except Exception as e:
        ...
        success, duration_ms, error = await self._timed_check(
            test_firestore_comprehensive
        )
        ...
        if not sla_compliant:
            error = (
                error
                or f"SLA violation: {duration_ms:.0f}ms > {self.latency_sla_ms:.0f}ms"
            )
