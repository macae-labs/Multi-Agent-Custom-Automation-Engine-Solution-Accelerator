...
class AppHealthMonitor:
    ...
    def register_health_checker(
        ...
        if not issubclass(checker_class, ProviderHealthChecker):
            raise TypeError(
                f"checker_class must extend ProviderHealthChecker, got {checker_class}"
            )
    ...
    def _get_active_providers(self) -> List[str]:
        ...
        return [
            ...
            if p.provider_id in self._health_checker_registry
        ]
    ...
    def _create_health_checker(
        ...
        if not checker_class:
            logger.warning(f"No health checker registered for provider: {provider_id}")
            return None
        ...
        try:
            ...
            logger.error(f"Failed to create health checker for {provider_id}: {e}")
            return None
    ...
    async def collect_provider_health(
        ...
        try:
            ...
            health = await asyncio.wait_for(
                checker.check_health(), timeout=timeout_seconds
            )
            return health
    ...
    def _calculate_health_score(
        ...
        healthy_count = sum(1 for h in provider_health.values() if h.is_healthy)
    ...
    async def collect_app_kpis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cosmos = CosmosMemoryContext(
                session_id=self.session_id or "health_check", user_id="system"
            )
            ...
            in_progress_plans = sum(
                ...
                and str(getattr(p.overall_status, "value", p.overall_status)).lower()
                == "in_progress"
            )
            completed_plans = sum(
                1
            ...
            completed_steps = sum(
                ...
                for s in all_steps
                if s.status
                and str(getattr(s.status, "value", s.status)).lower() == "completed"
            )
            failed_steps = sum(
                1
                for s in all_steps
                if s.status
                and str(getattr(s.status, "value", s.status)).lower() == "failed"
            )
            ...
            success_rate = (
                (completed_plans / (completed_plans + failed_plans) * 100)
                if (completed_plans + failed_plans) > 0
                else 0.0
            )
            ...
            return {
                ...
                "error_rate": (failed_steps / total_steps * 100)
                if total_steps > 0
                else 0.0,
                "completion_rate": completion_rate,
                "success_rate": success_rate,
                "avg_steps_per_plan": (total_steps / total_plans)
                if total_plans > 0
                else 0.0,
                "engagement_ratio": (active_sessions / len(all_sessions) * 100)
                if all_sessions
                else 0.0,
