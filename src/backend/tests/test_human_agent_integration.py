...
async def test_human_agent_loads_tools():
    ...
    agent_type_str = AgentFactory._agent_type_strings.get(
        AgentType.HUMAN, "human_agent"
    )
    ...
    if tools_file:
        ...
        if config_tool_names:
            ...
            assert common_tools, (
                f"None of the tools from {tools_file} were loaded in the Human agent"
            )
...
async def test_human_agent_has_methods():
    ...
    assert hasattr(agent, "handle_human_feedback"), (
        "Human agent does not have handle_human_feedback method"
    )
    assert hasattr(agent, "provide_clarification"), (
        "Human agent does not have provide_clarification method"
    )
