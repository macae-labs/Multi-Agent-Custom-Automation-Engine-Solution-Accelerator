...
def azure_env_available():
    ...
    if missing:
        logger.warning(
            f"Missing required environment variables for Azure tests: {missing}"
        )
        return False
    return True
...
skip_if_no_azure = pytest.mark.skipif(
    not azure_env_available(), reason="Azure environment not configured"
)
...
async def test_azure_project_client_connection():
    ...
    assert conn_str_env is not None, (
        "AZURE_AI_AGENT_PROJECT_CONNECTION_STRING environment variable not set"
    )
...
async def test_create_hr_agent():
    ...
    agent = await AgentFactory.create_agent(
        agent_type=AgentType.HR, session_id=TEST_SESSION_ID, user_id=TEST_USER_ID
    )
    ...
    assert isinstance(agent._agent, AzureAIAgent), (
        "The _agent attribute of HR agent is not an AzureAIAgent"
    )
...
async def test_hr_agent_loads_tools_from_json():
    ...
    if tools_file:
        ...
        config_tool_names = [
            tool.get("name", "") for tool in tools_config.get("tools", [])
        ]
        ...
        for required_tool in [
            ...
            found = any(
                required_tool.lower() in tool_name for tool_name in agent_tool_names
            )

            # If not found with exact matching, try a more lenient approach
            if not found:
                found = any(
                    tool_name in required_tool.lower()
                    or required_tool.lower() in tool_name
                    for tool_name in agent_tool_names
                )

            assert found, (
                f"Required tool '{required_tool}' was not loaded by the HR agent"
            )
            if found:
    ...
    # Log success
    logger.info(
        f"Successfully verified HR agent loaded {len(agent._tools)} tools from JSON configuration"
    )
...
async def test_hr_agent_has_system_message():
    ...
    found_term = next(
        (term for term in hr_terms if term.lower() in system_message.lower()), None
    )
    assert found_term, (
        "System message for HR agent does not contain any HR-related terms"
    )

    # Log success with the actual system message
    logger.info(
        f"Successfully verified system message for HR agent: '{system_message}'"
    )
...
async def test_hr_agent_tools_existence():
    ...
    config_tool_names = [
        tool.get("name", "").lower() for tool in tools_config.get("tools", [])
    ]
    for tool_name in critical_tools:
        assert tool_name.lower() in config_tool_names, (
            f"Critical tool '{tool_name}' not in HR tools JSON config"
        )
    ...
    assert len(agent_tool_names) >= 25, (
        f"HR agent should have at least 25 tools, but only has {len(agent_tool_names)}"
    )

    logger.info(
        f"Successfully verified HR agent has {len(agent_tool_names)} tools available"
    )
...
async def test_hr_agent_direct_tool_execution():
    ...
    try:
        ...
        # Verify the result
        assert orientation_result is not None, (
            "No result returned from orientation tool"
        )
        assert "Jane Doe" in str(orientation_result), (
            "Employee name not found in orientation tool result"
        )
        assert "April 25, 2025" in str(orientation_result), (
            "Date not found in orientation tool result"
        )
        ...
        benefits_result = await agent.invoke_tool(
            benefits_tool_name, {"employee_name": "John Smith"}
        )
        ...
        assert "John Smith" in str(benefits_result), (
            "Employee name not found in benefits tool result"
        )
        ...
        assert "Alice Brown" in str(leave_result), (
            "Employee name not found in leave request tool result"
        )

        logger.info(
            "Successfully executed HR agent tools directly through the agent instance"
        )
...
async def test_hr_agent_function_calling():
    ...
    try:
        ...
        prompt = (
            "I need to schedule an orientation session for Jane Doe on April 25, 2025"
        )
        ...
        if hasattr(agent._agent, "get_chat_history"):
            ...
            for tool in agent._tools:
                if hasattr(tool, "metadata") and hasattr(
                    tool.metadata, "kernel_function_definition"
                ):
            ...
            completion = await client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": agent._system_message},
                    message,
                ],
                tools=tools,
                tool_choice="auto",
            ...
            if completion.choices and completion.choices[0].message.tool_calls:
                ...
                logger.info(
                    f"Azure OpenAI used function calling with {len(tool_calls)} tool calls"
                )
                ...
                for tool_call in tool_calls:
                    ...
                    if "schedule_orientation" in function_name.lower():
                        ...
                        assert "employee_name" in args_dict, (
                            "employee_name parameter missing"
                        )
                        assert "Jane Doe" in args_dict["employee_name"], (
                ...
                assert "orientation" in content.lower(), (
                    "Response doesn't mention orientation"
                )
                assert "Jane Doe" in content, (
                    "Response doesn't mention the employee name"
                )
