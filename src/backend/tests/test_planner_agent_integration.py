...
from models.messages_kernel import InputTask, Plan, Step, PlanStatus, StepStatus
...
class PlannerAgentIntegrationTest(unittest.TestCase):
    ...
    async def test_plan_generation_content(self):
        ...
        self.assertTrue(
            any(term in plan.initial_goal.lower() for term in marketing_terms)
        )
    ...
    async def test_handle_plan_clarification(self):
        ...
        args = KernelArguments(
            session_id=self.session_id, human_clarification=clarification
        )
    ...
    async def test_hr_agent_selection(self):
        ...
        self.assertTrue(
            has_hr_steps, "No steps assigned to HrAgent for an onboarding task"
        )
        ...
        if no_marketing_steps:
            ...
            print(
                f"❌ TEST FAILED: Found {len(marketing_steps)} steps incorrectly assigned to MarketingAgent"
            )
        ...
        if onboarding_steps:
            ...
            self.assertEqual(
                onboarding_steps[0].agent,
                "HrAgent",
                "The step containing 'onboard' was not assigned to HrAgent",
            )
            if onboard_correct:
                print(
                    "✅ TEST PASSED: Steps containing 'onboard' are assigned to HrAgent"
                )
            else:
                print(
                    f"❌ TEST FAILED: Step containing 'onboard' assigned to {onboarding_steps[0].agent}, not HrAgent"
                )
        ...
        # If no specific "onboard" step but we have steps, the first should likely be HrAgent
            ...
            if first_step_correct:
                ...
                print(
                    f"❌ TEST FAILED: First step assigned to {first_agent}, not HrAgent"
                )
