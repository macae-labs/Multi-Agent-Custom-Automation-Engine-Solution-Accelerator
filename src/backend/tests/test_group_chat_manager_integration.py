...
class GroupChatManagerIntegrationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if (
            not Config.COSMOSDB_ENDPOINT
            or Config.COSMOSDB_ENDPOINT == "https://localhost:8081"
        ):
        ...
        print(
            f"  - Cosmos DB: {Config.COSMOSDB_DATABASE} at {Config.COSMOSDB_ENDPOINT}"
        )
    ...
    async def test_handle_input_task(self):
        ...
        input_task = InputTask(
            session_id=self.session_id, user_id=self.user_id, description=self.test_task
        )
    ...
    async def test_human_feedback(self):
        ...
        result = await self.group_chat_manager.handle_human_feedback(
        ...
        self.assertEqual(
            updated_step.human_approval_status, HumanFeedbackStatus.accepted
        )
        self.assertEqual(
            updated_step.human_feedback,
        ...
        self.assertTrue(
            any("perform action" in msg.content.lower() for msg in messages)
        )
    ...
    async def test_execute_next_step(self):
        ...
        result = await self.group_chat_manager.execute_next_step(
            self.session_id, plan.id
        )
        ...
        updated_steps = await self.memory_store.get_steps_for_plan(
            plan.id, self.session_id
        )

        # Verify at least one step has changed status
        action_requested_steps = [
            step for step in updated_steps if step.status == StepStatus.action_requested
        ]
    ...
    async def test_run_group_chat(self):
        ...
        user_input = (
            "What's the best way to create a social media campaign for our new product?"
        )
    ...
    async def test_conversation_history_generation(self):
        ...
        if len(steps) > 1:
            ...
            conversation_history = (
                await self.group_chat_manager._generate_conversation_history(
                    steps, second_step.id, plan
                )
            )
