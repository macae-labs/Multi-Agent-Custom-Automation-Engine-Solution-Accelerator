...
async def test_create_real_agent(agent_type, expected_agent_class, ai_project_client):
    ...
    agent = await AgentFactory.create_agent(
        agent_type=agent_type, session_id=TEST_SESSION_ID, user_id=TEST_USER_ID
    )
    ...
    assert isinstance(agent, expected_agent_class), (
        f"Agent is not an instance of {expected_agent_class.__name__}"
    )

    # Verify that the agent is or contains an AzureAIAgent
    assert hasattr(agent, "_agent"), (
        f"{agent_type_name} agent does not have an _agent attribute"
    )
    assert isinstance(agent._agent, AzureAIAgent), (
        f"The _agent attribute of {agent_type_name} agent is not an AzureAIAgent"
    )

    # Verify that the agent has a client attribute that was created by the project_client
    assert hasattr(agent._agent, "client"), (
        f"{agent_type_name} agent does not have a client attribute"
    )
    ...
    assert agent._session_id == TEST_SESSION_ID, (
        f"{agent_type_name} agent has incorrect session_id"
    )

    # Check that the agent has the correct user_id
    assert agent._user_id == TEST_USER_ID, (
        f"{agent_type_name} agent has incorrect user_id"
    )

    # Log success
    logger.info(
        f"Successfully created a real {agent_type_name} agent using project_client"
    )
    return agent
...
async def test_agent_loads_tools_from_json(agent_type, ai_project_client):
    ...
    logger.info(
        f"Testing tool loading for agent type: {agent_type_name} (type string: {agent_type_str})"
    )
    ...
    if tools_file:
        ...
        agent_tool_names = [
            t.name.lower() if hasattr(t, "name") and t.name else ""
        ...
        if config_tool_names:
            ...
            common_tools = [
                ...
                if any(
                    config_name in name or name in config_name
                    for config_name in config_tool_names
                )
            ]

            assert common_tools, (
                f"None of the tools from {tools_file} were loaded in the {agent_type_name} agent"
            )
            logger.info(f"Found common tools: {common_tools}")

    # Log success
    logger.info(
        f"Successfully verified {agent_type_name} agent loaded {len(agent._tools)} tools"
    )
    return agent
...
async def test_human_agent_can_execute_method(ai_project_client):
    ...
    agent = await AgentFactory.create_agent(
        agent_type=AgentType.HUMAN, session_id=TEST_SESSION_ID, user_id=TEST_USER_ID
    )
    ...
    assert hasattr(agent, "handle_action_request"), (
        "Human agent does not have handle_action_request method"
    )
