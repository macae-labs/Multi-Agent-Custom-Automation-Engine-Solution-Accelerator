...
async def test_strategic_orchestrator():
    ...
    try:
        ...
        logger.info(f"  Overall Health: {'✓ HEALTHY' if snapshot.overall_health else '✗ DEGRADED'}")
        ...
        snapshot = {
            ...
            "provider_health": {
                ...
                "aws_s3": {"is_healthy": False, "response_time_ms": 0.0, "error_message": "Connection timeout"},
            },
            "app_kpis": {
                "active_sessions": 3,
                "total_plans": 45,
    ...
    try:
        ...
        logger.info(f"✓ Trends retrieved: {len(trends.get('timeline', []))} data points")
        
        if trends.get('summary'):
            logger.info(f"  Summary: {trends['summary'].get('trend', 'unknown')}")
            logger.info(f"  Avg Health Score: {trends['summary'].get('avg_health_score', 0):.1f}%")
    except Exception as e:
    ...
    try:
        analysis = await agent.analyze_business_context(
            health_snapshot=snapshot if isinstance(snapshot, dict) else snapshot.to_dict(),
            trends=trends,
        )
        ...
        logger.info(f"  Unhealthy Providers: {health_status.get('unhealthy_providers', [])}")
        ...
        for issue in issues:
            ...
            logger.info(f"  {severity_icon} [{issue.get('severity')}] {issue.get('category')}: {issue.get('issue')}")
        ...
        for i, action in enumerate(actions, 1):
            priority_icon = {"CRITICAL": "🔴", "HIGH": "🟠", "MEDIUM": "🟡"}.get(action.get("priority"), "⚪")
            logger.info(f"  {priority_icon} [{action.get('priority')}] {action.get('action')}")
            ...
            logger.info(f"      Autonomous: {'Yes ✓' if action.get('can_execute_autonomously') else 'No (manual review)'}")
        ...
        logger.info("\nAutonomy Decision:")
        logger.info(f"  Escalate: {'Yes' if autonomy.get('escalate') else 'No (Self-Heal Attempt)'}")
        ...
        logger.info(f"  Message Type: '{autonomy.get('message_type')}'")
        logger.info(f"  Severity: '{autonomy.get('escalate') and 'CRITICAL' or 'ADVISORY'}'")
