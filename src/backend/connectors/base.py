...
class ConnectorConfig:
    ...
    graph_client_id: str = field(default_factory=lambda: os.getenv("MICROSOFT_GRAPH_CLIENT_ID", ""))
    graph_client_secret: str = field(default_factory=lambda: os.getenv("MICROSOFT_GRAPH_CLIENT_SECRET", ""))
    graph_tenant_id: str = field(default_factory=lambda: os.getenv("MICROSOFT_GRAPH_TENANT_ID", ""))

    # Database settings (for employee records, etc.)
    hr_database_url: str = field(default_factory=lambda: os.getenv("HR_DATABASE_URL", ""))
    ...
    calendar_api_url: str = field(default_factory=lambda: os.getenv("CALENDAR_API_URL", ""))

    # Demo mode - when True, connectors return simulated responses
    demo_mode: bool = field(default_factory=lambda: os.getenv("CONNECTOR_DEMO_MODE", "true").lower() == "true")

    def is_graph_configured(self) -> bool:
        """Check if Microsoft Graph API is properly configured."""
        return all([self.graph_client_id, self.graph_client_secret, self.graph_tenant_id])
...
def get_connector_config() -> ConnectorConfig:
    ...
    if _config is None:
        ...
        if _config.demo_mode:
            logger.info("Connectors running in DEMO MODE - returning simulated responses")
        else:
            logger.info("Connectors running in PRODUCTION MODE - connecting to real services")
    return _config
...
class BaseConnector(ABC):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"{self.service_name} connector initialized successfully")
            return success
