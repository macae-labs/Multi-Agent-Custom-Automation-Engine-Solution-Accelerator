...
class ToolRegistry:
    ...
    def get_tools_for_agent(agent_type: str) -> List[ToolDefinition]:
        ...
        return [
            tool for tool in TOOL_DEFINITIONS.values() if tool.agent_type == agent_type
        ]
    ...
    def _expand_enabled_tools(enabled_tools: Optional[List[str]]) -> set[str]:
        ...
        for token in enabled:
            ...
            # Provider id
            provider_id = (
                token if token in provider_ids else legacy_alias_to_provider.get(token)
            )
            if provider_id:
    ...
    def get_tools_for_agent_and_profile(
        ...
        for tool in candidate_tools:
            if (
                tool.tool_id in expanded_enabled
                or tool.provider_id in active_provider_set
            ):
