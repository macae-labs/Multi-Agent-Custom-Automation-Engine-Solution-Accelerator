...
class TestHealthCheckSummary:
    ...
    def test_add_mixed_results_with_failure(self):
        ...
        assert (
            summary.status is False
        )  # Overall status should be False due to one failure
    ...
    def test_add_default_check(self):
        ...
        assert (
            summary.results["Default"].message
            == "This is the default check, it always returns True"
        )
    ...
    def test_add_exception_with_complex_error(self):
        ...
        summary = HealthCheckSummary()
        complex_error = ValueError(
            "Invalid configuration: timeout=None, expected positive integer"
        )
...
class TestHealthCheckMiddleware:
    ...
    async def test_check_method_with_successful_custom_check(self):
        ...
        # Ensure it has the __await__ attribute by checking the coroutine and consuming it
        coro = success_check()
        assert hasattr(coro, "__await__"), "Should be awaitable"
    ...
    async def test_check_method_with_failing_custom_check(self):
        ...
        if "failing" in result.results:
            ...
            assert (
                result.results["failing"].message == "Custom failure"
                or "not a coroutine function" in result.results["failing"].message
            )
    ...
    async def test_check_method_with_exception_in_check(self):
        ...
        with patch("backend.middleware.health_check.logging.error") as mock_logger:
            ...
            if "exception" in result.results:
                ...
                assert (
                    "Check failed with exception" in message
                    or "not a coroutine function" in message
                )
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_json_response.assert_called_once_with(
                        mock_encoded_data, status_code=200
                    )
