...
class TestFindFirstAvailableTeam:
    ...
    async def test_find_first_available_team_rfp_available(self):
        ...
        mock_team_service.get_team_configuration = AsyncMock(
...
class TestCreateRAIAgent:
    ...
    async def test_create_rai_agent_success(
        ...
        assert (
            call_args[1]["agent_description"]
            == "A comprehensive research assistant for integration testing"
        )
        assert (
            "You are RAIAgent, a strict safety classifier for professional workplace use"
            in call_args[1]["agent_instructions"]
        )
        assert call_args[1]["use_reasoning"] is True  # Changed to True
...
class TestRaiSuccess:
    ...
    async def test_rai_success_content_safe(self, mock_get_response, mock_create_agent):
        ...
        mock_create_agent.assert_called_once_with(
            self.mock_team_config, self.mock_memory_store
        )
    ...
    async def test_rai_success_content_unsafe(
        ...
        # Execute
        result = await rai_success(
            "Unsafe content", self.mock_team_config, self.mock_memory_store
        )
    ...
    async def test_rai_success_agent_creation_fails(self, mock_create_agent):
        ...
        # Execute
        result = await rai_success(
            "Test content", self.mock_team_config, self.mock_memory_store
        )
...
class TestRaiValidateTeamConfig:
    ...
    def setup_method(self):
        ...
        self.sample_team_config = {
            ...
            "starting_tasks": [
                {"name": "Task 1", "prompt": "Complete the first task"},
                {"name": "Task 2", "prompt": "Complete the second task"},
            ],
        }
    ...
    async def test_rai_validate_team_config_valid(self, mock_uuid, mock_rai_success):
        ...
        # Execute
        is_valid, message = await rai_validate_team_config(
            self.sample_team_config, self.mock_memory_store
        )
    ...
    async def test_rai_validate_team_config_empty_content(self):
        ...
        # Execute
        is_valid, message = await rai_validate_team_config(
            empty_config, self.mock_memory_store
        )
    ...
    async def test_rai_validate_team_config_exception(
        ...
        assert (
            message
            == "Unable to validate team configuration content. Please try again."
        )
        mock_logging.error.assert_called_once()
    ...
    async def test_rai_validate_team_config_malformed_structure(
        ...
        # Execute
        is_valid, message = await rai_validate_team_config(
            malformed_config, self.mock_memory_store
        )
    ...
    async def test_rai_validate_team_config_partial_content(
        ...
        # Execute
        is_valid, message = await rai_validate_team_config(
            partial_config, self.mock_memory_store
        )
