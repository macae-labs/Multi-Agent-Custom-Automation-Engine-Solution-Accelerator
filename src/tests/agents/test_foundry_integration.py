...
class TestFoundryAgentIntegration:
    ...
    async def test_bing_search_functionality(self):
        ...
        try:
            if not agent.bing or not agent.bing.connection_name:
                pytest.skip(
                    "Bing configuration not available - skipping Bing search test"
                )
    ...
    async def test_rag_search_functionality(self):
        ...
        try:
            if not agent.search or not agent.search.connection_name:
                pytest.skip(
                    "Azure AI Search configuration not available - skipping RAG test"
                )
            ...
            assert any(
                ...
                for indicator in ["heavy rain", "Logistics", "2023-07-18"]
            ), (
    ...
    async def test_code_interpreter_functionality(self):
        ...
        try:
            if not agent.enable_code_interpreter:
                pytest.skip(
                    "Code Interpreter not enabled - skipping code interpreter test"
                )

            query = (
                "Can you write and execute Python code to calculate the factorial of 5?"
            )
    ...
    async def test_agent_handles_missing_configs_gracefully(self):
        ...
        try:
            ...
            pytest.fail(
                f"Agent should handle missing configs gracefully, but failed with: {e}"
            )
        finally:
            await agent.close()
